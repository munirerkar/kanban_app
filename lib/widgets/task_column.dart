  ...
  Widget build(BuildContext context) {
    return Consumer2<TaskViewModel, UserViewModel>(
      builder: (context, taskViewModel, userViewModel, child) {
        if (taskViewModel.isLoading) return const Center(child: CircularProgressIndicator());
        if (taskViewModel.errorMessage != null) return Center(child: Text(taskViewModel.errorMessage!));
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return Dismissible(
                ...
                direction: taskViewModel.isSelectionMode ? DismissDirection.none : DismissDirection.horizontal,
                ...
                child: GestureDetector(
                  // 👇 UZUN BASINCA: Seçim modunu başlat
                    ...
                    child: AnimatedContainer(
                      duration: const Duration(milliseconds: 300), // Renk değişimi animasyonu
                      margin: const EdgeInsets.only(bottom: 12),
                      padding: const EdgeInsets.all(16),
                      decoration: BoxDecoration(
                        color: isSelected ? Colors.blue.withOpacity(0.1) : Theme.of(context).colorScheme.surface,
                        border: isSelected ? Border.all(color: Colors.blue, width: 2) : null, // Seçiliyse mavi çerçeve
                        borderRadius: BorderRadius.circular(16),
                        boxShadow: isSelected ? [] : [ // Seçiliyse gölgeyi kaldır (düz görünsün)
